...
class GFSlack extends GFFeedAddOn {
	 ...
	 * @see GFForms::$version
	 */
	const LAST_REAUTHENTICATION_VERSION = '1.13';
	...
	/**
	 * Flags whether this add-on should use legacy API calls.
	 *
	 * @since 1.13
	 * @var bool
	 */
	protected $uses_legacy_api = false;
	...
	public function display_notices() {
		...
		$message = sprintf(
			...
			esc_html__(
				'On February 24, 2021, Slack is discontinuing access to previous versions of their API. %1$sPlease disconnect and reconnect%2$s to reauthenticate with Slack and ensure the continued functionality of your form feeds.',
				'gravityformsslack'
			),
	...
	public function groups_for_feed_setting() {
		...
		// Get fresh data from Slack and prepare choices for the feed settings.
		$channels = $this->request_all_channels( 'get_groups', $this->uses_legacy_api ? 'groups' : 'channels' );
	...
	public function initialize_api( $auth_token = null ) {
		...
		$this->uses_legacy_api = $this->requires_api_reauthentication();
		...
		if ( $this->uses_legacy_api ) {
			require_once gf_slack()->get_base_path() . '/includes/class-slack-legacy-api.php';
		}
		...
		$slack = $this->uses_legacy_api ? new Slack_Legacy_API( $auth_token ) : new GF_Slack_API( $auth_token );
